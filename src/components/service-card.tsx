  ...
  return (
    ...
    >
      <motion.div
        ...
        className={cn(
          "group relative h-full bg-white rounded-[2rem] border p-5 md:p-8 flex flex-col overflow-hidden transition-all duration-300",
      ...
      >
        ...
        {/* Description */}
        {service.note && (
          <div className="relative z-10 mb-3 md:mb-4 px-3 py-1.5 md:py-2 bg-yellow-50 border border-yellow-200 rounded-lg text-yellow-800 text-[10px] md:text-xs font-bold flex items-center gap-2">
             <span>{service.note}</span>
          </div>
        )}

        <p className="relative z-10 text-slate-500 text-sm mb-4 md:mb-8 flex-grow leading-relaxed font-medium line-clamp-3 md:line-clamp-none">
          {service.description}
        </p>
          ...
          <div className="relative z-10 mb-4 md:mb-8 space-y-2 md:space-y-3">
            {service.features.slice(0, 3).map((feature, i) => (
              <div key={i} className="flex items-start gap-2 md:gap-3">
                <div className={cn("p-0.5 md:p-1 rounded-full shrink-0 mt-0.5", bgLight)}>
        ...
        {service.objective && !service.features && (
          <div className={cn("relative z-10 mb-4 md:mb-8 p-3 md:p-4 rounded-xl border border-transparent group-hover:border-white/50 transition-colors", bgLight)}>
            <div className="flex items-start gap-2 md:gap-3">
              <TrendingUp className="w-4 h-4 md:w-5 md:h-5 shrink-0 mt-0.5" style={{ color: accentColor }} />
              <p className="text-xs md:text-sm text-slate-700 font-semibold italic">
                {service.objective}
              </p>
            </div>
          </div>
        ...
        <div className="relative z-10 mt-auto pt-4 md:pt-6 border-t border-slate-100 flex flex-col gap-3 md:gap-4">
          ...
          <div className="flex items-center gap-3">
             {quantity > 0 ? (
                <div className="flex items-center gap-3 w-full">
                    ...
                    <span className="font-black text-xl flex-1 text-center">{quantity}</span>
                    <Button 
                        size="icon"
                        className="rounded-xl h-12 w-12 text-white shadow-md"
                        style={{ backgroundColor: accentColor }}
